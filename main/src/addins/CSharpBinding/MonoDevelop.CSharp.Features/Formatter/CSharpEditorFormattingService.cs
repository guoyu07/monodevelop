...
{
	internal partial class CSharpEditorFormattingService
	{
		...
		public bool SupportsFormattingOnTypedCharacter(Document document, char ch)
		{
			...
			var autoFormattingOnTyping = true; //options.GetOption(FeatureOnOffOptions.AutoFormattingOnTyping, LanguageNames.CSharp);
		...
		internal static bool TokenShouldNotFormatOnReturn(SyntaxToken token)
		{
			return !token.IsKind(SyntaxKind.CloseParenToken) || !token.Parent.IsKind(SyntaxKind.UsingStatement);
		}

		internal static async Task<bool> TokenShouldNotFormatOnTypeCharAsync(
		...
		public async Task<IList<TextChange>> GetFormattingChangesAsync(Document document, char typedChar, int caretPosition, CancellationToken cancellationToken)
		{
			...
			var autoFormattingCloseBraceOff = false;
		...
		static internal async Task<SyntaxToken> GetTokenBeforeTheCaretAsync(Document document, int caretPosition, CancellationToken cancellationToken)
		{
		...
		static internal async Task<IList<TextChange>> FormatTokenAsync(Document document, SyntaxToken token, IEnumerable<IFormattingRule> formattingRules, CancellationToken cancellationToken)
		{
			var root = await document.GetSyntaxRootAsync(cancellationToken).ConfigureAwait(false);
			var options = await document.GetOptionsAsync(cancellationToken).ConfigureAwait(false);
		...
		static internal SmartTokenFormatter CreateSmartTokenFormatter(OptionSet optionSet, IEnumerable<IFormattingRule> formattingRules, SyntaxNode root)
		{
			return new SmartTokenFormatter(optionSet, formattingRules, (CompilationUnitSyntax)root);
		}

		internal static async Task<IList<TextChange>> FormatRangeAsync(
		...
		internal static bool IsInvalidToken(char typedChar, SyntaxToken token)
		{
			string text = null;
			if (IsInvalidToken(token, ref text))
			{
		...
		internal static bool IsInvalidToken(SyntaxToken token, ref string text)
		{
		...
		internal static bool IsInvalidTokenKind(SyntaxToken token)
		{
